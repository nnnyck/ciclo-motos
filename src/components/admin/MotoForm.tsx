...
export default function MotoForm({ editingMoto, onSubmit, onCancel }: Props) {
  ...
  const [ano, setAno] = useState<number | ''>(editingMoto?.ano || '');
  const [preco, setPreco] = useState(editingMoto?.preco || '');
  const [descricao, setDescricao] = useState(editingMoto?.descricao || '');
  const [quilometragem, setQuilometragem] = useState<number | ''>(editingMoto?.quilometragem || '');
  ...
  return (
    <div className="p-4 border rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <input className="w-full p-2 border" placeholder="Ano" type="number" value={ano as any} onChange={(e) => setAno(e.target.value ? Number(e.target.value) : '')} />
        <input className="w-full p-2 border" placeholder="Preço" value={preco} onChange={(e) => setPreco(e.target.value)} />
        <input className="w-full p-2 border" placeholder="Quilometragem" type="number" value={quilometragem as any} onChange={(e) => setQuilometragem(e.target.value ? Number(e.target.value) : '')} />
